...
import { type ApiError, type ReceiveStockInput, toApiError } from "@vet/shared";
...
// key per call. Multi-line: each row becomes its own /inventory/receive call; the shared `reason`
...
const DEFAULTS: ReceiveForm = { lines: [{ ...EMPTY_LINE }], reason: "" };

/** Per-line server-error extractor — prefers field-level messages, falls back to the envelope. */
...
export function ReceiveStockDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Per-line server error keyed by line id (so it survives remove/reindex).
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const results = await Promise.allSettled(
      values.lines.map((ln) => {
        const body: ReceiveStockInput = {
          productId: ln.productId,
          quantity: ln.quantity,
          ...(reason ? { reason } : {}),
        };
        return receive.mutateAsync(body);
      }),
    );
    const okIdx: number[] = [];
      ...
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? t("admin.common.saving") : t("inventory.receive.submit")}
          </Button>
        </div>
      </form>
    </Dialog>
