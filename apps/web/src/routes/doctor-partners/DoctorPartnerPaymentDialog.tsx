...
import { useRecordDoctorPartnerPayment } from "@/queries/doctorPartners";

// Doctor-partner payments are always money leaving — only the immediate methods (incl. cheque).
...
export function DoctorPartnerPaymentDialog({
  open,
  doctorPartnerId,
  onClose,
}: {
  ...
  const record = useRecordDoctorPartnerPayment(doctorPartnerId);
  ...
  const onSubmit = handleSubmit((values) => {
    const cheque =
      values.method === "cheque"
        ...
        : {};
    const input: DoctorPartnerPaymentInput = {
  ...
  return (
    <Dialog open={open} onClose={onClose} title={t("doctorPartners.payment.title")} className="max-w-md">
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        <Field label={t("doctorPartners.payment.amount")} error={errors.amount?.message}>
          <Input
            type="number"
            step="0.01"
            min="0"
        ...
        <Field label={t("doctorPartners.payment.method")} error={errors.method?.message}>
          <Controller
            name="method"
            control={control}
            render={({ field }) => (
        ...
        <Field label={t("doctorPartners.payment.notes")} error={errors.notes?.message}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={record.isPending}>
            {record.isPending ? t("doctorPartners.payment.submitting") : t("doctorPartners.payment.submit")}
          </Button>
        </div>
      </form>
    </Dialog>
