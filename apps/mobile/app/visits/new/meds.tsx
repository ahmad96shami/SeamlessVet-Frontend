...
import { Pill as PillIcon, Search, Warn } from "@/components/icons";
 ...
 * are capped at on-hand (the Mo3.3 guard re-checks at confirm); prices preview the
 * catalog selling price (M29 dropped per-contract medication pricing — the server
 * remains the pricing authority at issuance via items=[] auto-assembly).
 */
export default function WizardMedsScreen() {
  ...
  const unitPrice = (r: FieldStockRow) => r.selling_price ?? 0;

  const medsTotal = useMemo(() => {
    let sum = 0;
    for (const r of stockRows) {
      const qty = cart[r.product_id];
      if (qty) sum += qty * (r.selling_price ?? 0);
    }
