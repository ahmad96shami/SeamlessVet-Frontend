...
/** POST /auth/centers — the active centers a phone belongs to (Mo13, tenant-routed login step one). */
export const centers = (phone: string): Promise<CenterOption[]> => fetchCenters(apiClient, phone);

/** POST /auth/center-by-code — resolve a center by its admin-issued code (registration routing). 404 if unknown. */
